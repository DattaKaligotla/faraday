...
function buildUserPrompt(input: JobInput): string {
  ...
  const memorySection = input.tenantMemory
    ? `\n\nTENANT CONTEXT (memory file maintained by the customer for tenant "${input.orgName ?? input.orgId ?? "unknown"}"):\n${input.tenantMemory}`
    : "";
...
RECORDED TOOL CALLS:
${JSON.stringify(input.toolCalls ?? [], null, 2)}${memorySection}

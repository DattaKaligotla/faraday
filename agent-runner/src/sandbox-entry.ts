 ...
 * `integrations/{uid}/requests/{requestId}/events/{auto-id}`. Doc ids are
 * Firestore auto-ids so two jobs on the same requestId can never overwrite
 * each other; the `seq` field is wall-clock monotonic across jobs and is
 * what consumers cursor on. The terminal event also updates `pr.status` on
 * the parent request doc.
 *
...
async function createFirestoreWriter(opts: WriterOpts): Promise<FirestoreWriter> {
  ...
  // `seq` is the cursor consumers (events GET endpoint, dashboard) order on.
  // It must be strictly increasing across retries on the same requestId, so we
  // anchor it at the job's wall-clock start and add a per-event index. Jobs
  // are seconds-to-minutes apart in practice, so seq stays monotonic even if
  // the events subcollection holds entries from multiple jobs.
  const jobStartMs = Date.now();
  let idx = 0;
  return {
    async write(ev: JobEvent) {
      const seq = jobStartMs + idx++;
      await eventsCol.add({
        seq,
        jobId: opts.jobId,
        ts: Timestamp.now(),
      ...
      if (phase) {
        ...
        await reqRef.set({ pr: { phase, lastEventAt: FieldValue.serverTimestamp() } }, { merge: true });
      }

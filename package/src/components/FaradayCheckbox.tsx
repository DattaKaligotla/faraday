...
export function FaradayCheckbox({ name, label, defaultChecked, required, value }: FaradayCheckboxProps) {
  return (
    <label style={fieldStyles.inlineRow}>
      <input type="checkbox" name={name} defaultChecked={defaultChecked} required={required} value={value} />
      <span>
        {label}

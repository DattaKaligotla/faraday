...
export function FaradayForm({ formId, title, submitLabel = "Submit", successMessage = "Thanks!" }: FaradayFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.form} noValidate={false}>
      ...
      <div style={styles.actions}>
        ...
        {state.kind === "ok" && <span style={{ ...styles.status, ...styles.statusOk }}>{state.message}</span>}
        {state.kind === "error" && <span style={{ ...styles.status, ...styles.statusError }}>{state.message}</span>}
      </div>
    </form>
  );
}

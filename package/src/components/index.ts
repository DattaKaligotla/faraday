...
export type { FaradayRadioGroupProps, FaradayRadioOption } from "./FaradayRadioGroup";
...
const asAny = <P>(c: React.ComponentType<P>) => c as unknown as React.ComponentType<Record<string, unknown>>;
...
export const DEFAULT_COMPONENTS: Record<string, ComponentRegistryEntry> = {
  ...
  FaradayBadge: {
    ...
    propsSchema: {
      ...
      variant: "string — one of: primary, secondary, success, warning, error (default: secondary)",
    },
  },
  FaradayToast: {
    ...
    propsSchema: {
      ...
      variant: "string — one of: info, warning, error, success (default: info)",
      duration: "number — auto-dismiss after N milliseconds; 0 = persistent (default: 4000)",
    },
  },
  FaradayForm: {
    ...
    propsSchema: {
      ...
      successMessage: "string — message shown next to the button after a successful submit (default: 'Thanks!')",
    },
  },
  FaradayTextInput: {
    component: asAny(FaradayTextInput),
    propsSchema: {
      name: "string (required) — the FormData key under which this field's value will be submitted",
  ...
  FaradaySelect: {
    ...
    propsSchema: {
      ...
      options: "array (required) — list of { label: string, value: string } option objects",
      defaultValue: "string — value of the option to select initially",
      placeholder: "string — disabled prompt option shown first when no defaultValue is set",
      required: "boolean — make the field required (default: false)",
    },
  },
  FaradayCheckbox: {
    ...
    propsSchema: {
      ...
      value: "string — value submitted when checked (default: 'on'; standard HTML)",
      required: "boolean — make the field required (default: false)",
    },
  },
  FaradayRadioGroup: {
    ...
    propsSchema: {
      name: "string (required) — the FormData key shared by all radios in the group",

...
export function extractTables(roots: HTMLElement[]): ExtractedTable[] {
  ...
  for (const root of roots) {
    ...
    for (const t of tables) {
      ...
      seen.add(t);
      const headerCells = Array.from(t.querySelectorAll("thead th, thead td")) as HTMLElement[];
      let columns: string[];
      ...
      if (headerCells.length > 0) {
        ...
        columns = Array.from(first.children).map((c) => (c as HTMLElement).textContent?.trim() ?? "");
  ...
  for (const root of roots) {
    ...
    for (const c of Array.from(candidates)) {
      ...
      for (const k of Array.from(c.children).slice(0, MAX_ENTRIES_PER_LIST)) {
        const text = ((k as HTMLElement).innerText ?? k.textContent ?? "").replace(/\s+/g, " ").trim();
        if (!text) continue;
 ...
 */
export function extractNeighborhoodStyles(store: AgentStore): NeighborhoodEntry[] {
...
export function isCardShaped(el: Element): boolean {
  const rect = (el as HTMLElement).getBoundingClientRect();

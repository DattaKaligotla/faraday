...
const BLOCKED_PATTERNS = [/url\s*\(/i, /expression\s*\(/i, /javascript\s*:/i, /behavior\s*:/i, /-moz-binding/i];
...
export function sanitizeStyleProps(properties: Record<string, string>, allowedProps: string[]): CSSProperties {
...
const DANGEROUS_URL_PREFIXES = [/^\s*javascript:/i, /^\s*data:text\/html/i, /^\s*vbscript:/i];
 ...
 */
export function sanitizeAttributes(attrs: Record<string, string>, allowlist: string[]): Record<string, string | null> {

...
function detectVisiblePageText(store: AgentStore, maxLen = 4000): string {
  ...
  const ids = Object.keys(store.getState().registry).filter((id) => !id.startsWith("__"));
...
export function buildSystemPrompt(store: AgentStore): string {
  ...
  return [
    ...
    'Page tree shows `[container] revenue-card` containing a `<table>` of monthly figures. The Structured-data section already extracted `[{Month: "Jan", Revenue: "$32k"}, …]`.',
    'Right move: one `injectHTML` call, `targetId: "revenue-card"`, `position: "inside-end"`. Look up `revenue-card` in the visual fingerprint — say `widthPx` is 360 and the median sibling card height is around 160. Wrapper `style="width:100%; box-sizing:border-box;"`, SVG `width="100%" viewBox="0 0 360 140" height="140"` (sized so title + axis labels + wrapper padding land near 160 total). Bar widths from `viewBox.width / bars.length`, NOT absolute pixels. Heights from the actual extracted values. Bar fill `currentColor` with stepping `fillOpacity`. Title row matches the card\'s heading weight from the fingerprint. Value labels above each bar in 11px. Wrapper padding matches the card\'s. One sentence: "Charted the monthly revenue inline."',
    'Failures to avoid: (a) a `FaradayCard` with prose "Monthly revenue chart" and no chart; (b) an unbounded SVG that grows to 800px tall and dominates the viewport.',
    "",
    '**Example 2 — "make this feel like Linear"**',
    'Page is light-mode, comfortable density. "Like Linear" cues: dark mode, indigo/violet accent, tight density, system-font (already inherited).',
    'Right move: one `applyTheme` call rewriting `--background` to a near-black, `--foreground` to a near-white, `--primary` to oklch around the violet hue (e.g. `oklch(0.65 0.2 280)`), `--border` to a subtle `rgba(255,255,255,0.08)`, `--muted-foreground` to a mid-gray. Don\'t touch component-level styles — let the cascade do the work. One sentence: "Switched to a Linear-style dark indigo theme."',
    "Failure: changing each card individually with `applyStyle` instead of one theme switch.",
    "",
    '**Example 3 — "add a contact form below the hero"**',
    "Page tree shows `[element] hero-cta` and a `[container] page-root` further out. No [container] adjacent to the hero — typical case.",
    'Right move: one `injectHTML` call, `targetId: "hero-cta"`, `position: "after"`, html containing a header ("Drop us a line"), 3 fields (name / email / message) each with explicit visible borders per the interactive-element rules, a submit button with filled background. Padding/border-radius read from the visual fingerprint of nearby elements. Pre-filled placeholders. One sentence: "Added a contact form right under the hero CTA."',
    "Failure: refusing because no container exists adjacent — `injectHTML` doesn't need a container, it can anchor to any modifiable.",
    "",
    ...
    "```",
    "",
    "## Targeting elements not in the tree above — `findElement`",
    "If the user references an element that ISN'T in the tree (the page tree only shows ids the engineer wrapped), call `findElement` first. The tool_result will contain `id` — pass that exact id as the `targetId` (or `containerId`) on your next tool call.",
    "",
    'Example for "make the sign-up button bigger":',
    '  Turn 1 → findElement({ query: "sign up button", role: "button" })',
    '  Tool result → {"ok":true,"id":"fdy-find-sign-up-button"}',
    '  Turn 2 → applyStyle({ targetId: "fdy-find-sign-up-button", properties: { fontSize: "20px", padding: "12px 24px" } })',
    "",
    "Use the optional inputs to disambiguate when there are similar-looking elements: `text` for a substring of the visible label, `role` for the kind of element (button, link, heading, input...), `near` to scope the search inside an existing tree id's subtree.",
    "",
    "If `findElement` returns `{ ok: false }`, refine the query or hints — don't invent an id that wasn't returned. **Prefer existing tree ids when they match the user's request.** Registered ids are more reliable than discovered ones; only call findElement when the element you need genuinely isn't in the tree.",
    "",
    ...
    Object.keys(snap.layoutModes).length > 0
      ? ["**Active layout overrides:**", "```json", JSON.stringify(snap.layoutModes, null, 2), "```", ""].join("\n")
      : "",
    ...
    "",
    "## Critical reminders — read these last, apply on every tool call",
    "These collapse the most important rules above into a checklist. If a rule here conflicts with anything earlier, this wins.",
    "",
    referenceCard
      ? `1. **Size new charts/widgets to ~${referenceCard.widthPx}×${referenceCard.heightPx}px** (the reference card on this page). Cap wrapper at \`max-width: ${referenceCard.widthPx}px\`, height within ~10% of ${referenceCard.heightPx}px. Don't size to a wide container's \`widthPx\` — match the cards.`
      : "1. **Size new charts/widgets to roughly the page's typical card dimensions.** Don't size to the parent container's full width.",
    '2. **For "at the bottom" / "below" / "add a graph here" with no specific element reference**: anchor `position: "inside-end"` on the outermost wide container, NOT `position: "after"` on a small card (CSS grid will jam it into the next column). For specific references like "next to the X card", anchor on X.',
    '3. **Wrapper centering**: every injected chart/widget gets `style="display: block; margin: 16px auto;"` on the outermost element so it centers in its parent.',
    '4. **Repositioning requests** ("move it", "center it", "too far left") = `removeInjection` the existing one + `injectHTML` again with the fix. Never reply "I\'ll move it" without firing both tools.',
    "5. **Replace, don't stack**: if a similar injection already exists in `Active HTML injections`, `removeInjection` the old one before `injectHTML` the new one.",
    '6. **Single-value charts are not charts.** A "histogram of cash balance" with one number = render a small KPI tile, not a full-card bar.',
    "7. **Use real data.** Pull values from the `Structured data found on the page` and `Repeating list patterns` sections. Don't fabricate numbers when actual data is on screen.",
    "8. **`#id` references in user messages are exact ids** — use them verbatim as `targetId`/`containerId`, no translation.",
    "9. **Sanitizer strips `<script>`, `on*=` handlers, `javascript:` URLs.** Don't include them; injected buttons can't have JS — use a caption to narrate intent if behavior matters.",
    "10. **You have visual context.** A screenshot of the page is in this conversation (and another one will follow your tool calls so you can verify the result). Use it: if the after-screenshot shows the change looks wrong, fix it with another tool call before replying.",
  ]
    .filter(Boolean)
    .join("\n");
}

...
export function Modifiable<T extends ElementType = "div">({
  ...
  const layoutStyle = isContainer && layoutOverride ? buildLayoutStyle(layoutOverride) : null;

  return (
    <>
      {injBefore.map(renderInjection)}
      <Tag ref={ref} id={id} {...rest} {...attributes} style={{ ...externalStyle, ...style, ...layoutStyle }}>

import type { AgentConnectionConfig, HtmlInjection, InsertedComponent, LayoutOverride, Override } from "../types";
...
export async function saveOverrides(
  ...
  payload: Snapshot & {
    ...
    pageContext?: unknown;
    /** Total stream time across the session (ms). `0` when the save originated
     * from inline-edit only — analytics aggregators filter zeros from
     * percentile math, so we send a real number rather than omit the field. */
    latencyMs: number;
    /** TTFT of the first chat turn in the session (ms). `0` when no chat
     * turn produced a text_delta (inline-only or pure tool_use). */
    firstTokenMs: number;
  },
): Promise<void> {
...
export async function loadOverrides(connection: AgentConnectionConfig): Promise<Snapshot | null> {

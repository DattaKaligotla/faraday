...
import { AgentStoreContext, AgentConnectionContext, AgentFormContext } from "./context";
...
export function UIAgentProvider({
  ...
  const apiUrl = apiUrlProp ?? getDevApiUrl();
  if (!publishableKey && !endpoint) {
    throw new Error("[Faraday] UIAgentProvider requires a `publishableKey`.");
  }
...
function ToastLayer({ store }: { store: AgentStore }) {
  const insertedList = useStore(store, (s) => s.insertedComponents["__faraday-toasts__"] ?? []);

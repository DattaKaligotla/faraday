...
describe("MockUIAgentProvider — initialOverrides", () => {
  it("pre-seeds text override so useModifiable reads it immediately", () => {
    render(
      <MockUIAgentProvider initialOverrides={{ hero: { text: "Mocked!" } }}>
        <TextDisplay id="hero" />
      </MockUIAgentProvider>,
    );
  ...
  it("pre-seeds style override", () => {
    render(
      <MockUIAgentProvider initialOverrides={{ hero: { style: { color: "red" } } }}>
        <TextDisplay id="hero" />
      </MockUIAgentProvider>,
    );
  ...
  it("pre-seeds visibility=false", () => {
    render(
      <MockUIAgentProvider initialOverrides={{ hero: { visible: false } }}>
        <TextDisplay id="hero" />
      </MockUIAgentProvider>,
    );
...
describe("MockUIAgentProvider — onApply", () => {
  it("fires onApply when an action is dispatched via useAgentSnapshot", async () => {
    ...
    function Driver() {
      const snap = useAgentSnapshot();
      return <button onClick={() => snap.apply({ type: "setText", targetId: "hero", text: "Updated" })}>go</button>;
    }
...
describe("runMockStream", () => {
  ...
  it("stops at done event", async () => {
    ...
    const handler = createMockStream(
      [{ type: "text_delta", delta: "Before" }, { type: "done" }, { type: "text_delta", delta: "After" }],
      { delayMs: 0 },
    );
    ...
    function Fixture() {
      ...
      return (
        <div>
          ...
          <button onClick={() => snap.apply({ type: "setText", targetId: "item", text: "changed" })}>apply</button>
        </div>
      );
    }

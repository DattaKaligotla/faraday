...
describe("store.apply — setText", () => {
  ...
  it("returns error for unknown targetId", () => {
    const store = makeStore();
    const result = store.getState().apply({ type: "setText", targetId: "nope", text: "x" });
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/Unknown targetId/);
  });
});
...
describe("store.apply — onAction callback", () => {
  it("fires callback with dispatched action", () => {
    const store = makeStore();
    const calls: unknown[] = [];
    store.getState().apply({ type: "setText", targetId: "title", text: "CB" }, (a) => calls.push(a));
...
describe("store.apply — applyStyle scope=descendants", () => {
  ...
  it("snapshot exposes currentDescendantStyle", () => {
    ...
    const entry = store
      .getState()
      .snapshot()
      .modifiables.find((m) => m.id === "sidebar");
...
describe("store.apply — setAttributes", () => {
  ...
  it("returns error when nothing survives sanitization", () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/no allowed attributes/);
  });
...
describe("store.apply — removeComponent / removeInjection", () => {
  ...
  it("removeComponent deletes from insertedComponents", () => {
    const store = setup();
    const result = store.getState().apply({ type: "removeComponent", instanceId: "comp-1" });
    expect(result.ok).toBe(true);
  ...
  it("removeComponent unknown instanceId returns error", () => {
    const store = makeStore();
    const result = store.getState().apply({ type: "removeComponent", instanceId: "nope" });
    expect(result.ok).toBe(false);
  ...
  it("removeInjection unknown injectionId returns error", () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/not on target/);
  });

...
export function ChatPanel({ onClose }: ChatPanelProps) {
  ...
  const insertedComponents = useStore(store, (state) => state.insertedComponents);
  ...
  // Session-level latency accumulators shipped on save. Cleared after a
  // successful save so the next edit cycle starts fresh.
  const streamMsAccumRef = useRef(0);
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      const timing = await streamAgentResponse({
        connection,
        store,
        userMessage: trimmed,
        signal: abortRef.current.signal,
      });
      streamMsAccumRef.current += timing.streamMs;
      if (firstTokenMsRef.current === null && timing.firstTokenMs !== null) {
        firstTokenMsRef.current = timing.firstTokenMs;
      }
    },
  ...
  const hasChanges =
    ...
    Object.keys(insertedComponents).some((k) => (insertedComponents[k]?.length ?? 0) > 0) ||
    ...
    async (email: string) => {
      ...
      try {
        ...
        await saveOverrides(connection, {
          ...
          latencyMs: Math.round(streamMsAccumRef.current),
          firstTokenMs: Math.round(firstTokenMsRef.current ?? 0),
        });
        // Reset for the next edit cycle so latencies don't carry across saves.
        streamMsAccumRef.current = 0;
    ...
    <div className={styles.panel} role="dialog" aria-label="Faraday UI Agent">
      <div className={styles.panelHeader}>
        ...
        <div className={styles.headerActions}>
          <button
            ...
            onClick={onSave}
            disabled={saveStatus === "saving" || (saveStatus === "idle" && !hasChanges)}
      ...
      <div ref={messagesRef} className={styles.messages} role="log" aria-live="polite">
        {messages.map((msg) => (
          <div key={msg.id} className={styles.message} data-role={msg.role}>
            {msg.content}
            {msg.streaming && msg.content === "" && <span className={styles.streamingDot} aria-hidden />}
          </div>
        ))}
      </div>

      <div className={styles.inputRow}>
        <VoiceInput onTranscript={onVoiceTranscript} className={styles.voiceBtn} />
        <textarea
          ref={textareaRef}
      ...
      {emailModalOpen && (
        ...
        >
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>Save these changes</div>

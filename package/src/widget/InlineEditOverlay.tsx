    ...
    function recompute() {
      ...
      if (topRect) {
        ...
        setDotPos({
          ...
          left: Math.min(window.innerWidth - dotSize - 8, topRect.right - dotSize / 2),
        });
      } else if (!dotPosRef.current) {
        // No Modifiable with a non-zero rect yet AND we've never positioned
        // the button — anchor to the top-right of the viewport so it's
        // always reachable. Previously this branch did `setDotPos(null)`,
        // which made the entire overlay (including an open chat panel) blink
        // out during a streaming turn whenever the host Modifiable briefly
        // had zero dimensions due to reflows / theme-var swaps / inserted-
        // component reconciliation. Now: keep whatever position we already
        // had, only fall back to a viewport corner on cold start.
        const dotSize = 32;
        setDotPos({
          top: 16,
          left: Math.max(8, window.innerWidth - dotSize - 16),
        });
      }
      // else: preserve the existing dotPos — don't nuke a working anchor
      // because a Modifiable transiently has zero dimensions.
    }
    ...
    <div data-faraday>
      ...
      {!collapsed && (
        <>
              ...
              >
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setCollapsed(false);
                    window.dispatchEvent(new CustomEvent("faraday:mention", { detail: { id } }));
                  }}
                >
                  #{id}
                </button>
          ...
          <div ref={refs.setFloating} style={{ ...floatingStyles, zIndex: 2147483646 }}>
...
function pickLabelPosition(
  ...
  for (const c of candidates) {
    const collides = (panel && rectsOverlap(c.viewport, panel)) || (dotRect && rectsOverlap(c.viewport, dotRect));
